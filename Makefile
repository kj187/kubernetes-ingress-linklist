...
GO111          ?= on
CGO_ENABLED    ?= 0
REPOSITORY     ?= kj187/kubernetes-ingress-linklist
REF            ?= master
...
go_build: info ## Build the binary file
	GO111MODULE=$(GO111) go mod vendor
	GO111MODULE=$(GO111) CGO_ENABLED=$(CGO_ENABLED) go build -o k8s-ingress-linklist -mod vendor src/cmd/kubernetes-ingress-linklist/main.go
